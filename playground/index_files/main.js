...
$(() => {
  ...
  if (navBar.length !== 0) {
    navBar = $('#asideNavigationBar').clone();
    let headerElement = document.querySelector('.mezzanine');
    if (!headerElement) {
      headerElement = document.querySelector('.gp-game-card');
    }
    window.addEventListener('scroll', () => {
      const rect = headerElement.getBoundingClientRect();
      if (rect.bottom <= 80) {
        $('.page-header-bottom').append(navBar);
      } else {
  ...
  */
  ...
  $('#prizeWinnersList .btn').on('click', () => {
    $('#prizeWinnersList .overage').toggleClass('hidden');
  });
});
